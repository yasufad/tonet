...
func Run() (err error) {
	...
	app.UsageText = `tonet [GLOBAL OPTIONS] [COMMAND] [COMMAND OPTIONS] [filename(s) or folder]
   ...
   Send multiple files:
      tonet send file1.txt file2.txt file3.txt
    or
      tonet send *.jpg

   Send everything in a folder:
      tonet send example-folder-name

   Send a file with a custom code:
      tonet send --code secret-code file.txt
   ...
   Receive a file using code:
	...
	app.Commands = []*cli.Command{
		{
			Name:        "send",
			Usage:       "send file(s), or folder (see options with tonet send -h)",
	...
	app.Flags = []cli.Flag{
		...
		&cli.StringFlag{Name: "relay", Value: models.DEFAULT_RELAY, Usage: "address of the relay", EnvVars: []string{"TONET_RELAY"}},
		&cli.StringFlag{Name: "relay6", Value: models.DEFAULT_RELAY6, Usage: "ipv6 address of the relay", EnvVars: []string{"TONET_RELAY6"}},
		&cli.StringFlag{Name: "out", Value: ".", Usage: "specify an output folder to receive the file"},
		&cli.StringFlag{Name: "pass", Value: models.DEFAULT_PASSPHRASE, Usage: "password for the relay", EnvVars: []string{"TONET_PASS"}},
		&cli.StringFlag{Name: "socks5", Value: "", Usage: "add a socks5 proxy", EnvVars: []string{"SOCKS5_PROXY"}},
		&cli.StringFlag{Name: "connect", Value: "", Usage: "add a http proxy", EnvVars: []string{"HTTP_PROXY"}},
...
Do you wish to continue to DISABLE the classic mode? (y/N) `)
				...
				if choice == "y" || choice == "yes" {
					...
					fmt.Print(`To send and receive, export the TONET_SECRET variable with the code phrase:

  Send:    TONET_SECRET=*** tonet send file.txt

  Receive: TONET_SECRET=*** croc` + "\n\n")
				} else {
...
func send(c *cli.Context) (err error) {
	...
	if len(fnames) == 0 {
		return errors.New("must specify file: tonet send [filename(s) or folder]")
	}
	...
	if !classicInsecureMode {
		...
		if (!(runtime.GOOS == "windows") && c.IsSet("code")) || os.Getenv("TONET_SECRET") != "" {
			crocOptions.SharedSecret = os.Getenv("TONET_SECRET")
			if crocOptions.SharedSecret == "" {
				fmt.Printf(`On UNIX systems, to send with a custom code phrase,
you need to set the environmental variable TONET_SECRET:

  TONET_SECRET=**** tonet send file.txt
...
func receive(c *cli.Context) (err error) {
	...
	classicInsecureMode := utils.Exists(getClassicConfigFile(true))
	if crocOptions.SharedSecret == "" && os.Getenv("TONET_SECRET") != "" {
		crocOptions.SharedSecret = os.Getenv("TONET_SECRET")
...
func relay(c *cli.Context) (err error) {
	log.Infof("starting tonet relay version %v", Version)
	debugString := "info"
	if c.Bool("debug") {
		debugString = "debug"
	}
